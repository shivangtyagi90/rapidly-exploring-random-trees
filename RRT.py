...
'''Function Definition : Point inside rrt ?'''
def point_inside_rrt(x,y):
    if x>RRT_x+RRT_border and x<RRT_x + RRT_width - RRT_border:
                if y>RRT_y+RRT_border and y < RRT_y + RRT_height - RRT_border:
                    return(True)
    return(False)


'''Function Definition : Random Point Generator inside rrt'''
def random_point():
    x_random = ri(RRT_x+RRT_border , RRT_x + RRT_width - RRT_border - 1)
    y_random = ri(RRT_y+RRT_border , RRT_y + RRT_height - RRT_border - 1 )
...
running = True
'''Button for rrt'''
pygame.draw.rect(screen,BLACK,(RRT_x,RRT_y,RRT_width,RRT_height),RRT_border)
...
'''End stores a set of destination point [Destination point- Green Point], 
        ...
        Due to huge number of pixels in this rrt'''
End=set()
...
ClickText()
DesText("steps : ",y=460)
DesText("Draw the obstacle than proceed further ")
while running:
        ...
        if m[0]==1:
            if point_inside_rec(B1.x,B1.y, B1.width, B1.height,x,y):
                    if level==1 and Start==[]:
                        level+=1
                        B1.colour=RED
                        DesText("Draw the initial point than proceed further")
                    elif level==2 and Start:
                        level+=1
                        B1.colour=GREEN
                        DesText("Draw the goal point you get the shortest path")
